...
```text
.artifacts/pack/dsh-browser-use-0.6.1.tgz
```

使用已安装的 CLI 添加该 tarball：

```powershell
dsh plugin --profile web add file:C:/path/to/dsh-browser-use/.artifacts/pack/dsh-browser-use-0.6.1.tgz
```

也可以使用 `deepseek-harness` 源码 checkout 中的 CLI：

```powershell
pnpm --dir C:\path\to\deepseek-harness dsh plugin --profile web add file:C:/path/to/dsh-browser-use/.artifacts/pack/dsh-browser-use-0.6.1.tgz
```
