...
function Section({remote}: { remote: any }) {
    ...
    return <div style={{maxWidth: 760}}>
        <div style={row}>
            <div>
                <div style={label}>浏览器类型</div>
                <div style={desc}>选择浏览器自动化使用的浏览器类型。</div>
            </div>
            <select
                aria-label="浏览器类型"
                ...
                style={{...input, ...control}}
                onChange={event => {
                    setDraft({...draft, browserType: event.target.value as BrowserType})
                }}
            >
        ...
        <div style={row}>
            <div>
                <div style={label}>无头模式</div>
                <div style={desc}>由后端启动浏览器时不显示浏览器窗口。</div>
            </div>
            <div style={{...control, display: 'flex', justifyContent: 'flex-end'}}>
                <Switch checked={draft.headless} onChange={headless => {
                    setDraft({...draft, headless})
                }}/>
            </div>
        </div>
        <div style={row}>
            <div>
                <div style={label}>浏览器位置</div>
                <div style={desc}>自动检索并保存浏览器可执行文件位置，也可手动指定。</div>
            </div>
            <div style={{...control, display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) auto', gap: 8}}>
                <input
                    ...
                    style={input}
                    onChange={event => {
                        setDraft({...draft, browserPath: event.target.value})
                    }}
                />
                <Button
                    variant="outline"
        ...
        <div style={{display: 'flex', justifyContent: 'flex-end', paddingTop: 20}}>
            <Button variant="primary" disabled={saving || picking} onClick={() => {
                void save()
...
export function apply(ctx: Context): void {
    ctx.slots.inject('settings.section', () => ctx.slots.register({
        ...
        order: 30,
        label: '浏览器自动化'
    }, () => <Section remote={(ctx as any).remote}/>))
}
