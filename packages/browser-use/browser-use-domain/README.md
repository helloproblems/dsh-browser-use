...
设置页切换浏览器类型会保留已填写的路径；若路径与新类型不匹配，需要清空路径以自动发现，或重新选择对应的可执行文件后才能保存。保存后由 Domain 热切换后端，无需重启。已有用户设置优先于组合默认值。

配置页根据 `ctx.locale.getLocale().active` 显示中文或英文，并订阅语言变化即时更新导航标题、字段说明、按钮和校验提示。`zh` 及其地区变体使用中文，其他语言回退英文。切换语言不会重新加载设置、清空未保存的配置或中断保存及目录选择。
...
- `backend` 是初始浏览器类型对应的注册表别名，属于组合配置；GUI 中的 `browserType` 可在运行时选择 Chrome 或 Edge。
- Edge 使用 Playwright MCP；启用两个后端插件后，可通过设置页热切换浏览器。
- 同一后端内的设置变更保持工具目录不变；切换后端成功时会注册目标后端工具并注销旧工具。目标不可用或重新配置失败时保留旧目录。
